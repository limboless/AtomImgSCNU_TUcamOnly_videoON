...
class Chameleon(cameraBase):
    ...
    def setGRAB_MODE(self):
        conf = self.camera.getConfiguration()
        # print(str(conf.grabMode))
        conf.grabMode = PyCapture2.GRAB_MODE.DROP_FRAMES
        self.camera.setConfiguration(conf)
    ...
    def startAcquisition(self):
        ...
        if self.readyCapture:
            ...
            if self.mode == self.SOFTWARE_TRIGGER:
                ...
                # self.camera.setConfiguration(numBuffers=3)
                # a = self.camera.getProperty()
                # print(a)
                # self.camera.setConfiguration
